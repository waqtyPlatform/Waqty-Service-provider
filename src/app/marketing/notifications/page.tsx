...
const fallbackNotifications = [
    {
        ...
        status: 'sent',
        message:
            'Celebrate Eid with us! Get 25% off on all services from Feb 10-20. Book now on our app or call us. Limited spots available!',
    },
    {
        id: 2,
        ...
        status: 'sent',
        message:
            'Hi {name}, just a friendly reminder that your {service} appointment is scheduled for tomorrow at {time}. See you soon!',
    },
    {
        id: 3,
        ...
        status: 'scheduled',
        message:
            'Dear {name}, as a VIP client, you get exclusive early access to our Spring Sale! Enjoy 30% off on all premium services. Valid March 1-15.',
    },
    {
        id: 4,
        title: 'We miss you! Come back for 20% off',
        channel: 'WhatsApp',
        ...
        status: 'sent',
        message:
            "Hi {name}, it's been a while! We'd love to see you again. Here's 20% off your next visit as a welcome back gift. Book now!",
    },
    {
        id: 5,
        title: 'Rate your experience',
        channel: 'SMS',
        audience: "Today's Completed",
        sent: 12,
        opened: 8,
        date: '2026-03-23',
        status: 'sent',
        message: "Thank you for visiting us today! We'd love your feedback. Rate your experience: Reply with 1-5.",
    },
    {
        id: 6,
        title: 'Happy Birthday! Gift inside',
        channel: 'Push',
        ...
        status: 'sent',
        message:
            'Happy Birthday {name}! As our gift to you, enjoy a FREE express facial on your next visit. Valid for 7 days.',
    },
];
...
const channelBadge: Record<string, 'info' | 'primary' | 'purple' | 'success'> = {
    SMS: 'info',
    Push: 'primary',
    Email: 'purple',
    WhatsApp: 'success',
...
export default function NotificationsPage() {
    ...
    const openDetail = (n: (typeof notifications)[0]) => {
        setSelected(n);
    ...
    const filteredRecipients = useMemo(() => {
        if (!selected) return [];
        const list = recipientData[Number(selected.id)] || [];
        let filtered = list;
        if (recipientFilter === 'opened') filtered = list.filter(r => r.opened);
        if (recipientFilter === 'not_opened') filtered = list.filter(r => !r.opened);
        if (recipientSearch)
            filtered = filtered.filter(r => r.name.toLowerCase().includes(recipientSearch.toLowerCase()));
        return filtered;
    ...
    return (
        <div style={s.page}>
            ...
            <div style={s.kpis}>
                <div style={s.kpi}>
                    <div style={s.kpiVal}>{totalSent}</div>
                    <div style={s.kpiLbl}>{t('mkt.lblTotalSent')}</div>
                </div>
                <div style={s.kpi}>
                    <div style={s.kpiVal}>{totalOpened}</div>
                    <div style={s.kpiLbl}>{t('mkt.lblTotalOpened')}</div>
                </div>
                <div style={s.kpi}>
                    <div style={s.kpiVal}>{totalSent > 0 ? Math.round((totalOpened / totalSent) * 100) : 0}%</div>
                    <div style={s.kpiLbl}>{t('mkt.lblOpenRate')}</div>
                </div>
            </div>

            <div style={s.toolbar}>
                <button style={s.addBtn} onClick={() => setIsAddOpen(true)}>
                    <Plus size={16} /> {t('mkt.btnCompose')}
                </button>
            </div>
            ...
            <DataGuard
                ...
                emptyTitle={t('mkt.lblNoNotifications') || 'No Notifications'}
                emptyDescription={t('mkt.msgNoNotificationsDesc') || 'Compose your first notification to get started.'}
                emptyAction={
                    <button style={s.addBtn} onClick={() => setIsAddOpen(true)}>
                        <Plus size={16} /> {t('mkt.btnCompose')}
                    </button>
                    ...
                    <tbody>
                        {notifications.map(n => (
                            ...
                            >
                                ...
                                <td style={s.td}>
                                    <Badge color={channelBadge[n.channel]} size="sm">
                                        {n.channel}
                                    </Badge>
                                </td>
                                ...
                                <td style={s.td}>
                                    <Badge color={statusBadge[n.status]} size="sm">
                                        {t(`mkt.lbl${n.status.charAt(0).toUpperCase() + n.status.slice(1)}`) ||
                                            n.status}
                                    </Badge>
                                </td>
                                <td style={{ ...s.td, textAlign: 'right' }} onClick={e => e.stopPropagation()}>
                                    <DropdownMenu
                                        ...
                                        items={[
                                            ...
                                            {
                                                label: t('mkt.lblEditNotification'),
                                                icon: <Edit size={14} />,
                                                onClick: () => openEdit(n),
                                            },
                                            {
                                                label: t('mkt.lblDeleteNotification'),
                                                destructive: true,
                                                icon: <Trash2 size={14} />,
            ...
            <SlideOver
                ...
                title={t('mkt.lblNotificationDetails')}
                footer={
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-3)' }}>
                        <Button variant="ghost" onClick={() => openDelete(selected!)}>
                            <Trash2 size={14} /> {t('mkt.lblDeleteNotification')}
                        </Button>
                        <Button
                            variant="ghost"
                            onClick={() => {
                        ...
                        >
                            <RefreshCw size={14} /> {t('mkt.btnResend')}
                        </Button>
                    ...
                    <div style={s.detailSection as React.CSSProperties}>
                        <div>
                            ...
                            >
                                ...
                                <Badge color={statusBadge[selected.status]}>
                                    {t(
                            ...
                            <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
                                <Badge color={channelBadge[selected.channel]} size="sm">
                        ...
                        <div>
                            ...
                            >
                                <span style={{ fontSize: 'var(--text-sm)', fontWeight: 'var(--font-semibold)' }}>
                                    {selected.opened} / {selected.sent} {t('mkt.lblOpened').toLowerCase()}
                                </span>
                                <span style={{ fontSize: 'var(--text-sm)', color: 'var(--text-tertiary)' }}>
                                    {selected.sent > 0 ? Math.round((selected.opened / selected.sent) * 100) : 0}%
                                </span>
                            </div>
                            <div style={s.progressContainer}>
                                <div
                                    style={{
                                        ...s.progressFill,
                                        width: `${selected.sent > 0 ? Math.min((selected.opened / selected.sent) * 100, 100) : 0}%`,
                                        background: 'var(--color-primary-500)',
                                    }}
                                />
                            </div>
                                ...
                                {[
                                    {
                                        key: 'all' as const,
                                        label: t('mkt.lblAll'),
                                        count: recipientData[Number(selected.id)]?.length || 0,
                                    },
                                    {
                                        key: 'opened' as const,
                                        label: t('mkt.lblOpened'),
                                        count: recipientData[Number(selected.id)]?.filter(r => r.opened).length || 0,
                                    },
                                    {
                                        key: 'not_opened' as const,
                                        label: t('mkt.lblNotOpened'),
                                        count: recipientData[Number(selected.id)]?.filter(r => !r.opened).length || 0,
                                    },
                                ].map(f => (
                                    <button
                                        ...
                                        style={{
                                            ...s.filterBtn,
                                            ...(recipientFilter === f.key ? s.filterBtnActive : {}),
                                        }}
            ...
            <SlideOver
                ...
                title={t('mkt.lblComposeNotification')}
                footer={
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-3)' }}>
            ...
            <SlideOver
                ...
                title={t('mkt.lblEditNotification')}
                footer={
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-3)' }}>
            ...
            <Modal
                ...
                title={t('mkt.lblDeleteNotification')}
                footer={
                    <div style={{ display: 'flex', gap: 'var(--space-3)', justifyContent: 'flex-end' }}>
                        ...
                        <Button variant="destructive" onClick={handleDelete}>
                            {t('mkt.lblDeleteNotification')}
                        </Button>
                    </div>
                }
            >
                <p style={{ color: 'var(--text-secondary)' }}>
                    {t('mkt.msgDeleteNotificationConfirm')} <strong>{selected?.title}</strong>
                </p>
            </Modal>
