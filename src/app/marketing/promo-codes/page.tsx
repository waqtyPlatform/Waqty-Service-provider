...
import { Plus, Search, Copy, Tag, Edit, Trash2, MoreVertical, Eye, Percent } from 'lucide-react';
...
const clientUsageData: Record<
    number,
    Array<{ name: string; phone: string; usedAt: string; orderAmount: number; discount: number }>
> = {
    1: [
...
const fallbackCodes = [
    {
        ...
        description: 'Welcome offer for new clients on their first booking.',
    },
    {
        id: 2,
        code: 'EID25',
        ...
        description: 'Special Eid discount for all services above minimum order.',
    },
    {
        id: 3,
        code: 'VIP10',
        ...
        description: 'Exclusive VIP discount. No minimum order required.',
    },
    {
        id: 4,
        code: 'FRIDAY40',
        ...
        description: 'Flash Friday deal — 40% off all services. Limited to 30 uses.',
    },
    {
        id: 5,
        code: 'REFER20',
        ...
        description: 'Referral program discount. Share with friends!',
    },
    {
        id: 6,
        code: 'SUMMER100',
        ...
        status: 'scheduled',
        createdAt: '2026-03-15',
        description: 'Summer launch promo. 100 EGP off orders above 500 EGP.',
    },
];

const statusBadge: Record<string, 'success' | 'info' | 'error' | 'neutral'> = {
    active: 'success',
    scheduled: 'info',
    exhausted: 'error',
};
...
export default function PromoCodesPage() {
    ...
    const openDetail = (c: (typeof codes)[0]) => {
        setSelected(c);
    ...
    const filteredClients = useMemo(() => {
        if (!selected) return [];
        const list = clientUsageData[Number(selected.id)] || [];
    ...
    const totalSaved = useMemo(() => {
        ...
        return (clientUsageData[Number(selected.id)] || []).reduce((a, c) => a + c.discount, 0);
    }, [selected]);
        ...
        <div style={s.page}>
            ...
            <div style={s.toolbar}>
                <div style={s.searchBox as React.CSSProperties}>
                    ...
                    <input
                        style={s.searchInput}
                        placeholder={t('mkt.phSearchCodes')}
                        value={search}
                        onChange={e => setSearch(e.target.value)}
                    />
                </div>
                <button style={s.addBtn} onClick={() => setIsAddOpen(true)}>
                    <Plus size={16} /> {t('mkt.btnGenerateCode')}
                </button>
            </div>
            <DataGuard
                loading={loading}
                ...
                emptyTitle={t('mkt.lblNoPromoCodes') || 'No Promo Codes'}
                emptyDescription={t('mkt.msgNoPromoCodesDesc') || 'Generate your first promo code to get started.'}
                emptyAction={
                    <button style={s.addBtn} onClick={() => setIsAddOpen(true)}>
                        <Plus size={16} /> {t('mkt.btnGenerateCode')}
                    </button>
                        ...
                        {filtered.map(c => (
                            ...
                            >
                                <td style={s.td}>
                                    <span style={s.code}>
                                        <Tag size={12} /> {c.code}{' '}
                                        <Copy size={12} style={s.copyBtn} onClick={e => handleCopy(c.code, e)} />
                                    </span>
                                </td>
                                <td
                                    style={{
                                ...
                                >
                                    {c.type === 'percentage' ? `${c.discount}%` : `${c.discount} ${t('mkt.lblEGP')}`}
                                </td>
                                <td style={s.td}>
                                    {c.uses}/{c.limit}
                                </td>
                                ...
                                <td style={{ ...s.td, textAlign: 'end' }} onClick={e => e.stopPropagation()}>
                                    <DropdownMenu
                                        ...
                                        items={[
                                            {
                                                label: t('mkt.lblPromoCodeDetails'),
                                                icon: <Eye size={14} />,
                                                onClick: () => openDetail(c),
                                            },
                                            {
                                                label: t('mkt.lblEditCode'),
                                                icon: <Edit size={14} />,
                                                onClick: () => openEdit(c),
                                            },
                                            {
                                                label: t('mkt.lblDeletePromoCode'),
                                                destructive: true,
                                                icon: <Trash2 size={14} />,
            ...
            <SlideOver
                ...
                title={t('mkt.lblPromoCodeDetails')}
                footer={
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-3)' }}>
                        <Button variant="ghost" onClick={() => openDelete(selected!)}>
                            <Trash2 size={14} /> {t('mkt.lblDeletePromoCode')}
                        </Button>
                        <Button onClick={() => openEdit(selected!)}>
                            <Edit size={14} /> {t('mkt.lblEditCode')}
                        </Button>
                    </div>
                }
            >
                        ...
                        >
                            ...
                            <Button
                                ...
                                onClick={() => {
                                    ...
                                    addToast('success', t('mkt.lblCopyCode'));
                                }}
                            >
                                <Copy size={14} /> {t('mkt.lblCopyCode')}
                            </Button>
                            <div style={{ marginTop: 'var(--space-2)' }}>
                                <Badge color={statusBadge[selected.status]}>
                                    {t(`mkt.lbl${selected.status.charAt(0).toUpperCase() + selected.status.slice(1)}`)}
                                </Badge>
                            </div>
                        </div>
                            ...
                            <div style={s.infoCard}>
                                ...
                                >
                                    {selected.type === 'percentage'
                                        ? `${selected.discount}%`
                                        : `${selected.discount} ${t('mkt.lblEGP')}`}
                                </div>
                            </div>
                            <div style={s.infoCard}>
                                <div style={s.infoLabel as React.CSSProperties}>{t('mkt.lblDiscountType')}</div>
                                <div style={s.infoValue}>
                                    {selected.type === 'percentage' ? t('mkt.lblPercentage') : t('mkt.lblFixedAmount')}
                                </div>
                            </div>
                            <div style={s.infoCard}>
                                <div style={s.infoLabel as React.CSSProperties}>{t('mkt.lblMinOrder')}</div>
                                <div style={s.infoValue}>
                                    {selected.minOrder > 0 ? `${selected.minOrder} ${t('mkt.lblEGP')}` : '—'}
                                </div>
                            </div>
                            <div style={s.infoCard}>
                        ...
                        <div>
                            ...
                            >
                                <span style={{ fontSize: 'var(--text-sm)', fontWeight: 'var(--font-semibold)' }}>
                                    {selected.uses} / {selected.limit} {t('mkt.lblUsed')}
                                </span>
                                <span style={{ fontSize: 'var(--text-sm)', color: 'var(--text-tertiary)' }}>
                                    {Math.round((selected.uses / selected.limit) * 100)}%
                                </span>
                            </div>
                            <div style={s.progressLg}>
                                <div
                                    style={{
                                        ...s.progressFillLg,
                                        width: `${Math.min((selected.uses / selected.limit) * 100, 100)}%`,
                                        background:
                                            selected.uses >= selected.limit
                                                ? 'var(--color-error)'
                                                : 'var(--color-primary-500)',
                                    }}
                                />
                            ...
                            >
                                {selected.limit - selected.uses} {t('mkt.lblRemaining')}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            >
                                <div style={s.sectionTitle as React.CSSProperties}>
                                    {t('mkt.lblClientsWhoUsed')} ({(clientUsageData[Number(selected.id)] || []).length})
                                </div>
                                <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)' }}>
                                    {t('mkt.lblTotalSaved')} {totalSaved} {t('mkt.lblEGP')}
                                </div>
                            </div>
                            ...
                            >
                                {filteredClients.length === 0 ? (
                                    ...
                                    >
                                        {clientUsageData[Number(selected.id)]
                                            ? t('mkt.msgNoClientsMatch')
                                            : t('mkt.msgNoUsageData')}
                                    </div>
                                ) : (
            ...
            <Modal
                ...
                title={t('mkt.lblGeneratePromoCode')}
                footer={
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-3)' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                        ...
                        <Select
                            ...
                            options={[
                                { label: t('mkt.lblActive'), value: 'active' },
                                { label: t('mkt.lblScheduled'), value: 'scheduled' },
                                { label: t('mkt.lblExhausted'), value: 'exhausted' },
                            ]}
                        />
                    </div>
                )}
            ...
            <Modal
                ...
                title={t('mkt.lblDeletePromoCode')}
                footer={
                    <div style={{ display: 'flex', gap: 'var(--space-3)', justifyContent: 'flex-end' }}>
                        ...
                        <Button variant="destructive" onClick={handleDelete}>
                            {t('mkt.lblDeletePromoCode')}
                        </Button>
                    </div>
                }
            >
                <p style={{ color: 'var(--text-secondary)' }}>
                    {t('mkt.msgDeletePromoCodeConfirm')} <strong>{selected?.code}</strong>
                </p>
            </Modal>
