...
const fallbackGroups = [
    {
        id: 1,
        name: 'Hair Services',
        services: ['Hair Cut', 'Hair Coloring', 'Keratin Treatment', 'Blow Dry', 'Hair Mask', 'Highlights'],
        color: '#F59E0B',
        active: true,
    },
    {
        id: 2,
        name: 'Skin & Facial',
        services: ['Classic Facial', 'HydraFacial', 'Chemical Peel', 'Microdermabrasion', 'Acne Treatment'],
        color: '#EC4899',
        active: true,
    },
    {
        id: 3,
        name: 'Body & Massage',
        services: ['Swedish Massage', 'Deep Tissue', 'Hot Stone', 'Full Body Scrub', 'Aromatherapy'],
        color: '#10B981',
        active: true,
    },
    {
        id: 4,
        name: 'Nail Care',
        services: ['Gel Manicure', 'Pedicure', 'Nail Art', 'Paraffin Wax', 'French Tips'],
        color: '#3B82F6',
        active: true,
    },
    {
        ...
        services: ['Bridal Makeup', 'Up Style', 'Henna', 'Full Body Treatment', 'Trial Session'],
        color: '#8B5CF6',
        active: false,
    },
];
...
export default function ServiceGroupsPage() {
    ...
    const { loading, error, refetch } = useApiQuery(() => marketingApi.getServiceGroups() as never, [], {
    ...
    const openDelete = (g: (typeof fallbackGroups)[0]) => {
        setSelectedGroup(g);
    ...
    const handleAdd = async () => {
        if (!formName.trim()) {
            addToast('error', t('mkt.msgGroupNameRequired'));
            return;
        }
        if (formServices.length === 0) {
            addToast('error', t('mkt.msgSelectOneService'));
            return;
        }
        try {
            await marketingApi.createServiceGroup({ name: formName, services: formServices, color: formColor });
            refetch();
        } catch {
            /* fallback to local */
        }
    ...
    const handleEdit = async () => {
        ...
        try {
            if (selectedGroup?.id && typeof selectedGroup.id === 'string') {
                await marketingApi.updateServiceGroup(selectedGroup.id, {
                    name: formName,
        ...
        setGroups(prev =>
            prev.map(g =>
                g.id === selectedGroup!.id ? { ...g, name: formName, color: formColor, services: formServices } : g
            )
        );
        setIsEditOpen(false);
    ...
    const handleDelete = async () => {
        try {
            if (selectedGroup?.id && typeof selectedGroup.id === 'string') {
                await marketingApi.deleteServiceGroup(selectedGroup.id);
                refetch();
            }
        } catch {
            /* fallback */
    ...
    const renderServiceSelector = () => (
        <div>
            ...
            >
                {t('mkt.lblAssignServices')}{' '}
                <span style={{ color: 'var(--text-tertiary)', fontWeight: 'normal' }}>
                    ({formServices.length} {t('mkt.lblSelected')})
                </span>
            </label>
            ...
            >
                {filteredServices.map(svc => {
                    ...
                    return (
                        <div
                            key={svc}
                            style={{ ...s.serviceItem, background: isSelected ? formColor + '10' : 'transparent' }}
                            onClick={() => toggleService(svc)}
                        >
                            <div
                                style={{
        ...
        <div style={s.page}>
            ...
            <div style={s.toolbar}>
                <button style={s.addBtn} onClick={openAdd}>
                    <Plus size={16} /> {t('mkt.btnNewGroup')}
                </button>
            </div>
            <DataGuard
                loading={loading}
                ...
                emptyTitle={t('mkt.lblNoGroups') || 'No Service Groups'}
                emptyDescription={t('mkt.msgNoGroupsDesc') || 'Create your first service group.'}
                emptyAction={
                    <button style={s.addBtn} onClick={openAdd}>
                        <Plus size={16} /> {t('mkt.btnNewGroup')}
                    </button>
                        ...
                        <div key={g.id} style={{ ...s.card, opacity: g.active ? 1 : 0.6 }}>
                            <div style={s.cardHead}>
                                <div style={s.nameArea as React.CSSProperties}>
                                    <div style={{ ...s.icon, background: g.color }}>
                                        <Layers size={16} />
                                    </div>
                                    <div>
                                        <div style={s.name}>{g.name}</div>
                                        <div style={s.count}>
                                            {g.services.length} {t('mkt.lblServices')}
                                        </div>
                                    </div>
                                </div>
                                <div style={s.actions}>
                                    ...
                                    >
                                        <div
                                            style={{
                                                ...s.toggleKnob,
                                                transform: g.active ? 'translateX(16px)' : 'translateX(0)',
                                            }}
                                        />
                                    </div>
                                    <button style={s.btnIcon} onClick={() => openEdit(g)}>
                                        <Edit size={12} />
                                    </button>
                                    <button
            ...
            <Modal
                ...
                title={t('mkt.lblCreateNewGroup')}
                footer={
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-3)' }}>
                        <Button variant="ghost" onClick={() => setIsAddOpen(false)}>
                            {t('rtn.btnBack')}
                        </Button>
                        <Button onClick={handleAdd}>{t('mkt.btnSaveGroup')}</Button>
                    </div>
                }
            >
                            ...
                            {colorOptions.map(c => (
                                <div
                                    ...
                                    style={{
                                        ...s.colorDot,
                                        background: c,
                                        borderColor: c === formColor ? 'var(--text-primary)' : 'transparent',
                                    }}
                ...
                footer={
                    <div style={{ display: 'flex', gap: 'var(--space-3)', justifyContent: 'flex-end' }}>
                        ...
                        <Button variant="destructive" onClick={handleDelete}>
                            {t('mkt.lblDeleteGroup')}
                        </Button>
                    </div>
                }
            >
                <p style={{ color: 'var(--text-secondary)' }}>
                    {t('mkt.msgDeleteGroupConfirm')} <strong>{selectedGroup?.name}</strong>
                </p>
            </Modal>
        </div>
    );
