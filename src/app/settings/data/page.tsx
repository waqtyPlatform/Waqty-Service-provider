...
const fallbackDataSettings: DataSettings = {
    autoBackup: true,
    lastExport: null,
    lastImport: null,
};
...
export default function DataSettingsPage() {
    ...
    const {
        ...
        error,
        refetch,
    } = useApiQuery<DataSettings>(
        () => Promise.resolve({ success: true, message: '', data: fallbackDataSettings }),
        [],
        { fallbackData: fallbackDataSettings }
    );

    const [autoBackup, setAutoBackup] = useState(fallbackDataSettings.autoBackup);

    React.useEffect(() => {
        if (dataSettings) {
            setAutoBackup(dataSettings.autoBackup);
        }
    }, [dataSettings]);
    ...
    const handleImport = async () => {
        ...
        try {
            if (settingsApi.importData) {
                await settingsApi.importData({});
            }
    ...
    return (
        <div style={cs.page}>
            <DataGuard
                ...
                emptyTitle={t('settings.data.title')}
                emptyDescription={t('settings.data.desc')}
                onRetry={refetch}
                skeletonCount={2}
                        ...
                        <Switch
                            ...
                            onChange={() => {
                                ...
                                addToast(
                                    'info',
                                    !autoBackup
                                        ? t('settings.data.toastAutoBackupEnabled')
                                        : t('settings.data.toastAutoBackupDisabled')
                                );
                            }}
                        />
                    </div>
            ...
            >
                <div>
                    ...
                    >
                        {t('settings.data.exportModalMsg2')}
                    </p>
                </div>
            </Modal>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                    ...
                    >
                        <div style={{ color: 'var(--text-tertiary)', marginBottom: 'var(--space-2)' }}>
                            {t('settings.data.dragDrop')}
                        </div>
                        <Button variant="outline" size="sm">
                            {t('settings.data.browseFiles')}
                        </Button>
                    </div>
                </div>
            </Modal>
