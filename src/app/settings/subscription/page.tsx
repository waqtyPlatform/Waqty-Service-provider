...
export default function SubscriptionPage() {
    ...
    const { data: apiPlans, refetch } = useApiQuery<SubscriptionPlan[]>(() => settingsApi.getSubscription(), [], {
        fallbackData: [],
    });

    const handleChangePlan = async (planName: string) => {
        try {
            await settingsApi.changePlan({ plan: planName, billing_cycle: billingCycle });
            refetch();
        } catch {
            // plan change failed silently
        }
    };
    ...
    const usageStats = [
        ...
        {
            label: t('settings.subscription.activeBranches'),
            value: '2 / 3',
            ...
            <div style={s.currentPlan}>
                <div style={s.currentPlanHeader}>
                    <div>
                        ...
                        >
                            {t('settings.subscription.currentPlanLabel')}
                        </div>
                        <div
                            style={{
                    ...
                    {(['monthly', 'annual'] as const).map(cycle => (
                        ...
                        >
                            ...
                            {cycle === 'annual' && (
                                ...
                                >
                                    {t('settings.subscription.save20')}
                                </span>
                            )}
                        </button>
                    ))}
                            ...
                            >
                                {plan.current ? (
                                    ...
                                    <>
                                        ...
                                        <ArrowRight size={16} className={lang === 'ar' ? 'mr-2' : 'ml-2'} />
                                    </>
                                )}
                            </Button>
                        </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                    ...
                    <div>
                        ...
                        <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)' }}>
                            {t('settings.subscription.visaEnding')}
                        </div>
                    </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
