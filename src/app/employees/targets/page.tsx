...
const fallbackTargets: TargetRow[] = [
    {
        id: 'E001',
        employee: 'Sara Ahmed',
        avatar: 'SA',
        color: '#10b981',
        targetRevenue: 50000,
        achievedRevenue: 45200,
        targetBookings: 130,
        achievedBookings: 124,
        bonus: 2000,
    },
    {
        id: 'E002',
        ...
        color: '#f59e0b',
        targetRevenue: 40000,
        achievedRevenue: 38400,
        targetBookings: 110,
        achievedBookings: 102,
        bonus: 1500,
    },
    {
        id: 'E003',
...
export default function TargetsPage() {
    ...
    const {
        ...
        error,
        refetch,
    } = useApiQuery<EmployeeTarget[]>(() => employeeExtApi.getTargets() as never, [], {
        fallbackData: fallbackTargets,
    });

    // Map API data to local format, or use fallback
    const initialTargets: TargetRow[] = useMemo(() => {
        if (apiTargets && apiTargets.length > 0) {
            const colors = ['#10b981', '#f59e0b', '#3b82f6', '#8b5cf6', '#ec4899', '#6366f1'];
            return apiTargets.map((t, i) => ({
                id: t.uuid,
                employee: t.employee?.name || `Employee ${i + 1}`,
                avatar: (t.employee?.name || 'XX')
                    .split(' ')
                    .map(n => n[0])
                    .join('')
                    .toUpperCase(),
                color: colors[i % colors.length],
                targetRevenue: t.type === 'revenue' ? t.target_value : 0,
                achievedRevenue: t.type === 'revenue' ? t.current_value : 0,
                targetBookings: t.type === 'bookings' ? t.target_value : 0,
                achievedBookings: t.type === 'bookings' ? t.current_value : 0,
                bonus: Math.round(t.tier_multiplier * 1000),
            }));
        }
    ...
    React.useEffect(() => {
        if (initialTargets.length > 0 && initialTargets !== fallbackTargets) {
    ...
    const avgAchievement = Math.round(
        targets.reduce((sum, row) => sum + (row.achievedRevenue / row.targetRevenue) * 100, 0) / targets.length
    );
    ...
    const getStatusBadge = (pct: number) => {
        if (pct >= 100)
            return { label: t('targets.achieved'), bg: 'var(--color-success-light)', color: 'var(--color-success)' };
        if (pct >= 90)
            return { label: t('targets.onTrack'), bg: 'var(--color-primary-100)', color: 'var(--color-primary-600)' };
        if (pct >= 75)
            return { label: t('targets.nearTarget'), bg: 'var(--color-warning-light)', color: 'var(--color-warning)' };
    ...
    const handleSetTarget = async () => {
        ...
        // Attempt API update
        try {
            const existing = apiTargets?.find(t => t.employee?.name === editForm.employee);
            if (existing) {
                await employeeExtApi.updateTarget(existing.uuid, {
                    target_value: Number(editForm.targetRevenue) || 0,
                    type: 'revenue',
                });
                refetch();
            }
    ...
    const kpis = [
        {
            ...
            label: t('targets.kpiAvgAchievement'),
        },
        {
        ...
        },
        {
            ...
            label: t('targets.kpiTotalTarget'),
        },
    ];
    ...
    return (
        <div style={{ ...s.page, direction: lang === 'ar' ? 'rtl' : 'ltr' }}>
            <DataGuard
                ...
                emptyTitle={t('targets.emptyTitle') || 'No targets set'}
                emptyDescription={t('targets.emptyDesc') || 'Set revenue and booking targets for your employees.'}
            >
                {/* KPIs */}
                <div style={s.kpis}>
                    {kpis.map((kpi, i) => (
                ...
                <div style={s.toolbar}>
                    ...
                    <Button onClick={() => setIsSetOpen(true)}>
                        <Edit size={16} /> {t('targets.setTarget')}
                    </Button>
                </div>
                    ...
                    <table style={s.table}>
                        <thead>
                            <tr>
                                ...
                                <th style={{ ...s.th, minWidth: 180 } as React.CSSProperties}>
                                    {t('targets.colProgress')}
                                </th>
                            ...
                            {targets.map(row => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td style={s.td}>
                                            {row.achievedBookings}/{row.targetBookings}
                                        </td>
                                        <td style={s.td}>
                                            <span
                                                style={{
                                        ...
                                        <td
                                            style={{
                                                ...s.td,
                                                fontWeight: 'var(--font-semibold)',
                                                color: pct >= 90 ? 'var(--color-success)' : 'var(--text-tertiary)',
                                            }}
                                        >
                                            <span dir="ltr">
            ...
            <Modal
                ...
                footer={
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-3)' }}>
                        <Button variant="ghost" onClick={() => setIsSetOpen(false)}>
                            {t('targets.cancel')}
                        </Button>
            ...
            >
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                    <Select
                        ...
                        options={[
                            { value: '', label: t('targets.selectEmployee') },
                            ...targets.map(r => ({ value: r.employee, label: r.employee })),
                        ]}
                    />
                    <Input
                        label={t('targets.targetRevLabel')}
                        type="number"
                        value={editForm.targetRevenue}
                        onChange={e => setEditForm({ ...editForm, targetRevenue: e.target.value })}
                    />
                    <Input
                        label={t('targets.targetBookLabel')}
                        type="number"
                        value={editForm.targetBookings}
                        onChange={e => setEditForm({ ...editForm, targetBookings: e.target.value })}
                    />
                    <Input
                        label={t('targets.bonusLabel')}
                        type="number"
