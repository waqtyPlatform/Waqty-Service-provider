...
import { Shield, Search, Edit, UserCog } from 'lucide-react';
import { Button, Modal, Select, useToast, Badge } from '@/components/ui';
...
const moduleList = ['dashboard', 'sales', 'transactions', 'returns', 'customers', 'employees', 'marketing', 'reports', 'settings'];
const modulesAr: Record<string, string> = { dashboard: 'لوحة القيادة', sales: 'المبيعات', transactions: 'المعاملات', returns: 'المسترجعات', customers: 'العملاء', employees: 'الموظفون', marketing: 'التسويق', reports: 'التقارير', settings: 'الإعدادات' };

const initialPerms: EmpPerm[] = [
    { id: 'E001', employee: 'Sara Ahmed', avatar: 'SA', color: '#10b981', role: 'Manager', roleAr: 'مشرف', overrides: 2, modules: Object.fromEntries(moduleList.map(m => [m, { view: true, create: m !== 'settings', edit: m !== 'settings', delete: m === 'sales' }])) },
    { id: 'E002', employee: 'Nora Ali', avatar: 'NA', color: '#f59e0b', role: 'Stylist', roleAr: 'مصمم', overrides: 0, modules: Object.fromEntries(moduleList.map(m => [m, { view: ['dashboard', 'sales', 'customers'].includes(m), create: false, edit: false, delete: false }])) },
    { id: 'E003', employee: 'Layla Hassan', avatar: 'LH', color: '#3b82f6', role: 'Stylist', roleAr: 'مصمم', overrides: 1, modules: Object.fromEntries(moduleList.map(m => [m, { view: ['dashboard', 'sales', 'customers'].includes(m), create: m === 'customers', edit: false, delete: false }])) },
    { id: 'E004', employee: 'Hana Youssef', avatar: 'HY', color: '#8b5cf6', role: 'Receptionist', roleAr: 'موظف استقبال', overrides: 0, modules: Object.fromEntries(moduleList.map(m => [m, { view: ['dashboard', 'sales', 'customers', 'transactions'].includes(m), create: m === 'sales' || m === 'customers', edit: false, delete: false }])) },
    { id: 'E005', employee: 'Reem Mohamed', avatar: 'RM', color: '#ec4899', role: 'Stylist', roleAr: 'مصمم', overrides: 0, modules: Object.fromEntries(moduleList.map(m => [m, { view: ['dashboard', 'sales', 'customers'].includes(m), create: false, edit: false, delete: false }])) },
    { id: 'E006', employee: 'Dina Kamal', avatar: 'DK', color: '#6366f1', role: 'Stylist', roleAr: 'مصمم', overrides: 3, modules: Object.fromEntries(moduleList.map(m => [m, { view: ['dashboard', 'sales', 'customers', 'marketing'].includes(m), create: m === 'marketing', edit: m === 'marketing', delete: false }])) },
];
...
const s: Record<string, React.CSSProperties> = {
    ...
    modalTableWrapper: { border: '1px solid var(--border-color)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' },
    modalTh: { padding: 'var(--space-2) var(--space-3)', textAlign: 'center', fontSize: 12, fontWeight: 'var(--font-semibold)', color: 'var(--text-tertiary)', background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border-color)' },
    modalTd: { padding: 'var(--space-2) var(--space-3)', textAlign: 'center', borderBottom: '1px solid var(--border-color)' },
};
...
export default function PermissionsPage() {
    ...
    return (
        <div style={{ ...s.page, direction: lang === 'ar' ? 'rtl' : 'ltr' }}>
            {/* Toolbar */}
            <div style={s.toolbar}>
                <div style={{ display: 'flex', gap: 'var(--space-3)', alignItems: 'center', flex: 1, flexWrap: 'wrap' }}>
                    <div style={s.searchBox as React.CSSProperties}>
                        <Search size={16} style={s.searchIcon as React.CSSProperties} />
                        <input style={s.searchInput} placeholder={t('perms.searchPlaceholder')} value={search} onChange={e => setSearch(e.target.value)} />
                    </div>
                    <Select
                        value={roleFilter}
            ...
            <div style={s.tableWrapper}>
                <table style={s.table}>
                    ...
                    <tbody>
                        {filtered.map(emp => {
                            const accessCount = Object.values(emp.modules).filter(m => m.view || m.create || m.edit || m.delete).length;
                            return (
                                <tr key={emp.id} style={{ transition: 'background 0.15s' }} onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-secondary)'} onMouseLeave={e => e.currentTarget.style.background = ''}>
                                    <td style={s.td}>
                                        <div style={s.empCell}>
                                    ...
                                    <td style={s.td}>
                                        <span style={{ ...s.badge, background: 'var(--color-primary-100)', color: 'var(--color-primary-600)' }}>
                                            {lang === 'ar' ? emp.roleAr : emp.role}
                                        </span>
                                    </td>
                                    <td style={s.td}>
                                        {emp.overrides > 0 ? (
                                            <span style={{ ...s.badge, background: 'var(--color-warning-light)', color: 'var(--color-warning)' }}>
            ...
            <Modal
                ...
                footer={
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-3)' }}>
                        <Button variant="ghost" onClick={() => setIsEditOpen(false)}>{t('perms.cancel')}</Button>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                        ...
                        <div style={s.modalTableWrapper}>
                            <table style={{ ...s.table, minWidth: 'auto' }}>
                                <thead>
                                    <tr>
                                        <th style={{ ...s.modalTh, textAlign: lang === 'ar' ? 'right' : 'left' } as React.CSSProperties}>{t('perms.module')}</th>
                                ...
                                <tbody>
                                    {moduleList.map(m => (
                                        <tr key={m}>
                                            <td style={{ ...s.modalTd, textAlign: lang === 'ar' ? 'right' : 'left', fontWeight: 'var(--font-medium)', fontSize: 13 }}>
                                                {lang === 'ar' ? modulesAr[m] : m.charAt(0).toUpperCase() + m.slice(1)}
                                            </td>
                                            {(['view', 'create', 'edit', 'delete'] as const).map(perm => (
                                                <td key={perm} style={s.modalTd}>
                                                    <input type="checkbox" checked={editingEmp.modules[m]?.[perm] ?? false} onChange={() => togglePerm(m, perm)} style={{ width: 18, height: 18, cursor: 'pointer' }} />
                                                </td>
                                            ))}
                                        </tr>
                                    ))}
