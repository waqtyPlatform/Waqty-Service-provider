...
export default function BranchManagementPage() {
    ...
    // API: fetch branches (ready for backend)
    const { loading, error, refetch } = useApiQuery(() => providerApi.getBranches() as never, [], { fallbackData: [] });

    const branches = [
        {
            name: t('branchMgt.downtown'),
            manager: 'Sara Ahmed',
            employees: ['Sara Ahmed', 'Nora Ali', 'Layla Hassan', 'Hana Youssef', 'Reem Mohamed', 'Dina Nabil'],
            color: '#10B981',
        },
        {
            name: t('branchMgt.mallOfArabia'),
            manager: 'Fatma Hosny',
            employees: ['Fatma Hosny', 'Maya Adel', 'Rana Fawzy'],
            color: '#3B82F6',
        },
        {
            name: t('branchMgt.newCairo'),
            manager: 'Amira Sayed',
            employees: ['Amira Sayed', 'Salma Karim', 'Nadia Omar', 'Yara Emad'],
            color: '#8B5CF6',
        },
    ];
                ...
                {branches.map(branch => (
                    <div key={branch.name} style={s.card}>
                        <div style={s.cardHead}>
                            <div style={{ ...s.icon, background: branch.color }}>
                                <Building2 size={20} />
                            </div>
                            <div>
