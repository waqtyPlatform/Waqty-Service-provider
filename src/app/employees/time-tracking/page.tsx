...
import { Clock, Users, AlertTriangle, TrendingUp, Search, Calendar } from 'lucide-react';
...
export default function TimeTrackingPage() {
    ...
    const kpis = [
        { icon: <Clock size={20} />, bg: 'var(--color-primary-50)', color: 'var(--color-primary-600)', value: avgHours, label: t('timeTrack.kpiAvgHours') },
        { icon: <Users size={20} />, bg: 'var(--color-success-100)', color: 'var(--color-success-600)', value: clockedIn, label: t('timeTrack.kpiClockedIn') },
        { icon: <AlertTriangle size={20} />, bg: 'var(--color-warning-100)', color: 'var(--color-warning-600)', value: lateToday, label: t('timeTrack.kpiLate') },
        { icon: <TrendingUp size={20} />, bg: 'var(--color-purple-100)', color: 'var(--color-purple-600)', value: weeklyOvertime, label: t('timeTrack.kpiOvertime') },
    ];
            ...
            <div style={s.toolbar}>
                <div style={{ display: 'flex', gap: 'var(--space-3)', alignItems: 'center', flex: 1, flexWrap: 'wrap' }}>
                    <div style={s.searchBox as React.CSSProperties}>
                        ...
                        <input style={s.searchInput} placeholder={t('timeTrack.searchPlaceholder')} value={search} onChange={e => setSearch(e.target.value)} />
                    </div>
                    <Select
                        value={statusFilter}
                    ...
                    <input
                        ...
                        style={{ height: 40, padding: '0 var(--space-3)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-lg)', background: 'var(--bg-primary)', fontSize: 'var(--text-sm)', color: 'var(--text-primary)' }}
                    />
                </div>
            </div>
                ...
                <table style={s.table}>
                    ...
                    <tbody>
                        {filtered.map(entry => {
                            ...
                            return (
                                <tr key={entry.id} style={{ transition: 'background 0.15s' }} onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-secondary)'} onMouseLeave={e => e.currentTarget.style.background = ''}>
                                    <td style={s.td}>
                                        <div style={s.empCell}>
                                    ...
                                    <td style={{ ...s.td, color: entry.overtime !== '-' ? 'var(--color-warning)' : 'var(--text-tertiary)' }}>{entry.overtime}</td>
                                    <td style={s.td}>
                                        <span style={{ ...s.badge, background: sc.bg, color: sc.color }}>{statusLabels[entry.status]}</span>
                                    </td>
                                </tr>
                            );
                        })}
