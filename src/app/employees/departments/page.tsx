...
import { employeeExtApi, type Department } from '@/lib/api';
...
export default function DepartmentsPage() {
    ...
    const {
        ...
        error,
        refetch,
    } = useApiQuery<Department[]>(() => employeeExtApi.getDepartments() as never, [], {
        fallbackData: fallbackDepartments,
    });

    const mappedDepts = useMemo(() => {
        if (apiDepartments && apiDepartments.length > 0) {
            const colors = ['#F59E0B', '#EC4899', '#10B981', '#3B82F6', '#8B5CF6', '#6B7280'];
            return apiDepartments.map((d, i) => ({
                id: i + 1,
                uuid: d.uuid,
                name: d.name,
                manager: d.manager?.name || t('departments.unassigned'),
                employees: d.staff_count,
    ...
    const [localDepts, setLocalDepts] = useState<typeof fallbackDepartments | null>(null);
    const departments = localDepts ?? mappedDepts;
    const setDepartments = setLocalDepts;
    ...
    const handleSaveAdd = async () => {
        ...
        try {
            await employeeExtApi.createDepartment({ name: formData.name, manager_uuid: null });
            refetch();
        } catch {
            /* fallback */
        }
    ...
    const handleSaveEdit = async () => {
        if (!formData.name) return addToast('error', t('departments.toastReqName'));
        try {
            const dept = selectedDept as (typeof departments)[0] & { uuid?: string };
            if (dept?.uuid) {
                await employeeExtApi.updateDepartment(dept.uuid, { name: formData.name });
                refetch();
            }
        } catch {
            /* fallback */
    ...
    const handleDelete = async () => {
        try {
            const dept = selectedDept as (typeof departments)[0] & { uuid?: string };
            if (dept?.uuid) {
                await employeeExtApi.deleteDepartment(dept.uuid);
                refetch();
            }
        } catch {
            /* fallback */
    ...
    const handleAddStaff = () => {
        ...
        const roleExtracted =
            availableStaffOptions
                .find(o => o.value === newStaffSelection)
                ?.label.split('(')[1]
                .replace(')', '') || 'Employee';
        const initials = newStaffSelection
            .split(' ')
            .map(n => n[0])
            .join('')
            .toUpperCase();
        ...
        addToast(
            'success',
            `${t('departments.toastAssignedPart1')}${newStaffSelection}${t('departments.toastAssignedPart2')}${selectedDept?.name}`
        );
    };
    ...
    return (
        ...
        >
            ...
            {filtered.length > 0 ? (
                ...
                >
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                        <thead>
                            <tr>
                                {[
                                    { label: t('departments.colDept'), key: 'dept' },
                                    { label: t('departments.colManager'), key: 'manager' },
                                    { label: t('departments.colStaffCount'), key: 'count' },
                                    { label: t('departments.colStatus'), key: 'status' },
                                    { label: t('departments.colActions'), key: 'actions' },
                                ].map(h => (
                                    <th
                                        key={h.key}
                                        style={{
                                ...
                                <tr key={d.id} className="hoverRow">
                                    ...
                                    >
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                            <Users size={14} /> {d.employees} {t('departments.users')}
                                        </div>
                                    </td>
                                    <td
                                        style={{
                                    ...
                                    >
                                        <DropdownMenu
                                            ...
                                            items={[
                                                {
                                                    label: t('departments.actionView'),
                                                    icon: <Eye size={14} />,
                                                    onClick: () => {
                                                        setSelectedDept(d);
                                                        setIsViewOpen(true);
                                                ...
                                                {
                                                    label: t('departments.actionEdit'),
                                                    icon: <Edit size={14} />,
                                                    onClick: () => {
                                                        setSelectedDept(d);
                                                        setFormData({
                                                            name: d.name,
                                                            manager: d.manager,
                                                            employees: d.employees.toString(),
                                                            color: d.color,
                                                        });
                                                        setIsEditOpen(true);
                                                    },
                                                ...
                                                {
                                                    label: t('departments.actionDelete'),
                                                    destructive: true,
                                                    icon: <Trash2 size={14} />,
                                                    onClick: () => {
                                                        setSelectedDept(d);
                ...
                <EmptyState
                    icon={<Briefcase size={32} color="var(--text-tertiary)" />}
            ...
            >
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                    <Input
                        label={t('departments.lblName')}
                        placeholder={t('departments.phName')}
                        value={formData.name}
                        ...
                        <div style={{ display: 'flex', gap: 'var(--space-2)', alignItems: 'flex-end' }}>
                            <div style={{ flex: 1 }}>
                                <Select
                                    ...
                                    options={[
                                        { label: t('departments.phEmployee'), value: '' },
                                        ...availableStaffOptions,
                                    ]}
                                />
                            </div>
                                ...
                                {newDeptStaff.map(staff => (
                                    ...
                                    >
                                        ...
                                        <Trash2
                                            ...
                                            onClick={() => setNewDeptStaff(newDeptStaff.filter(s => s.id !== staff.id))}
                                        />
                                    </div>
                                ))}
                            </div>
                ...
                {selectedDept && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
                            ...
                            <div>
                                ...
                                <p style={{ fontSize: 'var(--text-sm)', color: 'var(--text-tertiary)' }}>
                                    {selectedDept.employees} {t('departments.activeStaff')}
                                </p>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            >
                                {t('departments.mgtInfo')}
                            </h3>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <span style={{ color: 'var(--text-tertiary)', fontSize: 'var(--text-sm)' }}>
                                        {t('departments.colManager')}
                                    </span>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <span style={{ color: 'var(--text-tertiary)', fontSize: 'var(--text-sm)' }}>
                                        {t('departments.payrollTag')}
                                    </span>
                                    <span style={{ fontWeight: 'var(--font-medium)', fontSize: 'var(--text-sm)' }}>
                                        {selectedDept.name.substring(0, 3).toUpperCase()}-XX
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            {staffList.length > 0 ? (
                                ...
                                <EmptyState
                                    icon={<Users size={24} color="var(--text-tertiary)" />}
                                    title={t('departments.emptyStaffTitle')}
                                    description={t('departments.emptyStaffDesc')}
                                />
                            )}
                        </div>
                    </div>
                ...
                footer={
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-3)' }}>
                        ...
                        <Button variant="destructive" onClick={handleDelete}>
                            {t('departments.btnRemoveAnyway')}
                        </Button>
                    </div>
                }
            >
                <p style={{ color: 'var(--text-secondary)' }}>
                    {t('departments.delConfirmMsg1')}
                    <strong>{selectedDept?.name}</strong>
                    {t('departments.delConfirmMsg2')}
                </p>
            </Modal>
