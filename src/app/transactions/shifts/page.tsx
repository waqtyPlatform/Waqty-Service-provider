...
export default function ShiftsPage() {
    ...
    const {
        ...
        error,
        refetch,
    } = useApiQuery<typeof fallbackData>(() => transactionApi.getShiftTotals() as never, [], {
    ...
    return (
        <div style={{ ...s.page, direction: lang === 'ar' ? 'rtl' : 'ltr' }}>
            <div style={s.tabBar}>
                {tabs.map(tab => (
                    <Link
                        key={tab.href}
                        href={tab.href}
                        style={{ ...s.tab, ...(tab.href === '/transactions/shifts' ? s.tabActive : {}) }}
                    >
                        {t(tab.labelKey)}
                    </Link>
                ))}
            ...
            <DataGuard
                ...
                emptyTitle={t('txn.shifts.emptyTitle') || 'No shifts found'}
                emptyDescription={t('txn.shifts.emptyDesc') || 'Shift records will appear here.'}
            >
                <table style={s.table}>
                    <thead>
                        <tr>
                            {[
                                ...
                                'txn.shifts.thOpen',
                                'txn.shifts.thClose',
                                'txn.shifts.thOpening',
                                'txn.shifts.thExpected',
                                'txn.shifts.thActual',
                        ...
                        {data.map(row => (
                            <tr key={row.id}>
                                ...
                                <td style={s.td}>
                                    ...
                                    >
                                        {row.status === 'closed'
                                            ? t('txn.shifts.statusClosed')
                                            : t('txn.shifts.statusOpen')}
                                    </span>
                                </td>
                            </tr>
                        ))}
