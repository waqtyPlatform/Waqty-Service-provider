...
const fallbackData: DailyRecord[] = [
    {
        date: '2026-03-25',
        revenue: 7850,
        cash: 5300,
...
export default function DailiesPage() {
    ...
    const {
        ...
        error,
        refetch,
    } = useApiQuery<DailyRecord[]>(() => transactionApi.getDailies() as never, [], { fallbackData: fallbackData });

    const data = (dailiesData ?? []) as DailyRecord[];
    ...
    return (
        <div style={{ ...s.page, direction: lang === 'ar' ? 'rtl' : 'ltr' }}>
            <div style={s.tabBar}>
                {tabs.map(tab => (
                    <Link
                        key={tab.href}
                        href={tab.href}
                        style={{ ...s.tab, ...(tab.href === '/transactions/dailies' ? s.tabActive : {}) }}
                    >
                        {t(tab.labelKey)}
                    </Link>
                ))}
            ...
            <div style={s.kpis}>
                ...
                <div style={s.kpi}>
                    <div style={s.kpiVal} dir="ltr">
                        {data.filter(d => d.status === 'closed').length}/{data.length}
                    </div>
                    <div style={s.kpiLbl}>{t('txn.dailies.closed')}</div>
                </div>
            </div>
            ...
            <DataGuard
                ...
                emptyTitle={t('txn.dailies.emptyTitle') || 'No daily reports'}
                emptyDescription={t('txn.dailies.emptyDesc') || 'Daily reports will appear here.'}
            >
                <table style={s.table}>
                    <thead>
                        <tr>
                            {[
                                'txn.dailies.thDate',
                                'txn.dailies.thRevenue',
                                'txn.dailies.thBreakdown',
                    ...
                    <tbody>
                        {data.map(row => (
                            <tr key={row.date}>
                                <td style={{ ...s.td, fontWeight: 'var(--font-medium)' }} dir="ltr">
                                    <span style={{ direction: 'ltr', display: 'inline-block' }}>{row.date}</span>
                                </td>
                                <td
                                    style={{
                                        ...s.td,
                                ...
                                <td style={s.td}>
                                    ...
                                    >
                                        <span style={{ color: 'var(--color-success)' }} dir="ltr">
                                            {Math.round((row.cash / row.revenue) * 100)}% {t('txn.dailies.lblCash')}
                                        </span>
                                        <span style={{ color: 'var(--color-info)' }} dir="ltr">
                                            {Math.round((row.card / row.revenue) * 100)}% {t('txn.dailies.lblCard')}
                                        </span>
                                        <span style={{ color: 'var(--color-warning)' }} dir="ltr">
                                            {Math.round((row.other / row.revenue) * 100)}% {t('txn.dailies.lblOther')}
                                        </span>
                                    </div>
                                    <div style={{ ...s.bar, transform: lang === 'ar' ? 'scaleX(-1)' : 'none' }}>
                                        <div
                                            style={{
                                                width: `${(row.cash / row.revenue) * 100}%`,
                                        ...
                                        <div
                                            style={{
                                                width: `${(row.card / row.revenue) * 100}%`,
                                        ...
                                        <div
                                            style={{
                                                width: `${(row.other / row.revenue) * 100}%`,
                                ...
                                <td style={s.td}>
                                    <span
                                        style={{
                                            ...
                                            color:
                                                row.status === 'closed' ? 'var(--color-success)' : 'var(--color-info)',
                                        }}
                                    >
                                        {row.status === 'closed'
                                            ? t('txn.dailies.statusClosed')
                                            : t('txn.dailies.statusOpen')}
                                    </span>
                                </td>
                            </tr>
                        ))}
