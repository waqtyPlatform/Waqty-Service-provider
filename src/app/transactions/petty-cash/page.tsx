...
const fallbackData = [
    ...
    },
    {
        ...
        date: '2026-03-16',
        category: 'Cleaning Supplies',
        description: 'Salon Floor Cleaner',
        vendor: 'CleanCo',
        amount: 65,
        approvedBy: '-',
        status: 'pending',
...
const catColors: Record<string, string> = {
    'Cleaning Supplies': '#3B82F6',
    'Office Supplies': '#8B5CF6',
    Refreshments: '#F59E0B',
    Maintenance: '#EF4444',
    'Staff Allowances': '#EC4899',
    Transport: '#10B981',
};
...
export default function PettyCashPage() {
    ...
    const {
        ...
        error,
        refetch,
    } = useApiQuery<typeof fallbackData>(() => transactionApi.getPettyCash() as never, [], {
        ...
        <div style={{ ...s.page, direction: lang === 'ar' ? 'rtl' : 'ltr' }}>
            <div style={s.tabBar}>
                {tabs.map(tab => (
                    <Link
                        key={tab.href}
                        href={tab.href}
                        style={{ ...s.tab, ...(tab.href === '/transactions/petty-cash' ? s.tabActive : {}) }}
                    >
                        {t(tab.labelKey)}
                    </Link>
                ))}
            ...
            <div style={s.kpis}>
                ...
                <div style={s.kpi}>
                    ...
                    <div style={s.kpiLbl}>{t('txn.petty.entries')}</div>
                </div>
            </div>
            ...
            <div style={s.toolbar}>
                ...
                <button style={s.addBtn}>
                    <Plus size={16} /> {t('txn.petty.newEntry')}
                </button>
            </div>
            ...
            <DataGuard
                ...
                emptyTitle={t('txn.petty.emptyTitle') || 'No petty cash entries'}
                emptyDescription={t('txn.petty.emptyDesc') || 'Petty cash entries will appear here.'}
            >
                <table style={s.table}>
                    <thead>
                        <tr>
                        ...
                        {filtered.map(row => (
                            <tr key={row.id}>
                                ...
                                <td style={s.td}>
                                    ...
                                    >
                                        {row.status === 'approved'
                                            ? t('txn.petty.statusApproved')
                                            : t('txn.petty.statusPending')}
                                    </span>
                                </td>
                            </tr>
                        ))}
