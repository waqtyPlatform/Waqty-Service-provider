...
export default function AdvancePaymentsPage() {
    ...
    const {
        ...
        error,
        refetch,
    } = useApiQuery<typeof fallbackData>(() => transactionApi.getAdvancePayments() as never, [], { fallbackData });

    const data = advancePayments ?? [];
    ...
    return (
        <div style={{ ...s.page, direction: lang === 'ar' ? 'rtl' : 'ltr' }}>
            <div style={s.tabBar}>
                {tabs.map(tab => (
                    <Link
                        key={tab.href}
                        href={tab.href}
                        style={{ ...s.tab, ...(tab.href === '/transactions/advance-payments' ? s.tabActive : {}) }}
                    >
                        {t(tab.labelKey)}
                    </Link>
                ))}
            ...
            <div style={s.kpis}>
                ...
                <div style={s.kpi}>
                    <div style={s.kpiVal}>{data.filter(d => d.status === 'partial').length}</div>
                    <div style={s.kpiLbl}>{t('txn.adv.pending')}</div>
                </div>
            </div>
            ...
            <DataGuard
                ...
                emptyTitle={t('txn.adv.emptyTitle') || 'No advance payments'}
                emptyDescription={t('txn.adv.emptyDesc') || 'Advance payments will appear here.'}
            >
                <table style={s.table}>
                    <thead>
                        <tr>
                            ...
                            <tr key={row.id}>
                                ...
                                <td
                                    style={{
                                        ...s.td,
                                        fontWeight: 'var(--font-semibold)',
                                        color: row.balance > 0 ? 'var(--color-warning)' : 'var(--color-success)',
                                    }}
                                    dir="ltr"
                                >
                                ...
                                <td style={s.td}>
                                    <span
                                        style={{
                                            ...
                                            color:
                                                row.status === 'paid' ? 'var(--color-success)' : 'var(--color-warning)',
                                        }}
                                    >
                                        {row.status === 'paid' ? t('txn.adv.statusPaid') : t('txn.adv.statusPartial')}
                                    </span>
                                </td>
                            </tr>
                        ))}
