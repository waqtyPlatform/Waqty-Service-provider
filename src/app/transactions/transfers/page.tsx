...
export default function TransfersPage() {
    ...
    const {
        ...
        error,
        refetch,
    } = useApiQuery<typeof fallbackData>(() => transactionApi.getTransfers() as never, [], {
        ...
        <div style={{ ...s.page, direction: lang === 'ar' ? 'rtl' : 'ltr' }}>
            <div style={s.tabBar}>
                {tabs.map(tab => (
                    <Link
                        key={tab.href}
                        href={tab.href}
                        style={{ ...s.tab, ...(tab.href === '/transactions/transfers' ? s.tabActive : {}) }}
                    >
                        {t(tab.labelKey)}
                    </Link>
                ))}
            ...
            <div style={s.kpis}>
                ...
                <div style={s.kpi}>
                    ...
                    <div style={s.kpiLbl}>{t('txn.transfers.count')}</div>
                </div>
                <div style={s.kpi}>
                    <div style={s.kpiVal} dir="ltr">
                        {data.filter(d => d.status === 'pending').length}
                    </div>
                    <div style={s.kpiLbl}>{t('txn.transfers.pending')}</div>
                </div>
            </div>
            ...
            <DataGuard
                ...
                emptyTitle={t('txn.transfers.emptyTitle') || 'No transfers'}
                emptyDescription={t('txn.transfers.emptyDesc') || 'Money transfers will appear here.'}
            >
                <table style={s.table}>
                    <thead>
                        <tr>
                            {[
                                'txn.petty.thID',
                                'txn.thDateTime',
                                'txn.transfers.thTime',
                                'txn.transfers.thFromTo',
                                'txn.thAmount',
                        ...
                        {data.map(row => (
                            <tr key={row.id}>
                                ...
                                <td style={s.td}>
                                    ...
                                    <ArrowRightLeft
                                        size={14}
                                        style={{ ...s.arrow, transform: lang === 'ar' ? 'scaleX(-1)' : 'none' }}
                                    />
                                ...
                                <td style={s.td}>
                                    ...
                                    >
                                        {row.status === 'completed'
                                            ? t('txn.transfers.statusDone')
                                            : t('txn.transfers.statusPending')}
                                    </span>
                                </td>
                            </tr>
                        ))}
