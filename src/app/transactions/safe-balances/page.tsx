...
export default function SafeBalancesPage() {
    ...
    const {
        ...
        error,
        refetch,
    } = useApiQuery<typeof fallbackSafes>(() => transactionApi.getSafeBalances() as never, [], {
        fallbackData: fallbackSafes,
    });

    const safes = safeBalances ?? [];
    ...
    return (
        <div style={{ ...s.page, direction: lang === 'ar' ? 'rtl' : 'ltr' }}>
            <div style={s.tabBar}>
                {tabs.map(tab => (
                    <Link
                        key={tab.href}
                        href={tab.href}
                        style={{ ...s.tab, ...(tab.href === '/transactions/safe-balances' ? s.tabActive : {}) }}
                    >
                        {t(tab.labelKey)}
                    </Link>
                ))}
            ...
            <DataGuard
                ...
                data={safes}
                onRetry={refetch}
                emptyIcon={<Vault size={48} />}
                emptyTitle={t('txn.safe.emptyTitle') || 'No safes found'}
                emptyDescription={t('txn.safe.emptyDesc') || 'Safe balances will appear here.'}
            >
                <div style={s.grid}>
                    {safes.map(safe => (
                        <div key={safe.name} style={s.card}>
                            <div style={s.cardHead}>
                                <div style={{ ...s.icon, background: safe.color }}>
                                    <Vault size={20} />
                                </div>
                                <div>
                            ...
                            <div style={s.rows}>
                                ...
                                <div style={s.row}>
                                    <span style={{ ...s.rowLabel, color: 'var(--color-success)' }}>
                                ...
                                <div style={{ ...s.row, borderBottom: 'none' }}>
                                    <span style={{ ...s.rowLabel, color: 'var(--color-error)' }}>
