...
const initialPackages: PackageData[] = [
    ...
    {
        id: 2,
        name: 'VIP Monthly Care',
        price: 1200,
        oldPrice: 1600,
        sessions: 6,
        sold: 67,
        services: ['Classic Facial', 'Swedish Massage', 'Manicure', 'Pedicure', 'Deep Conditioning', 'Eye Brow'],
        status: 'active',
        validity: '30',
        color: '#8B5CF6',
    },
    {
        ...
        sessions: 4,
        sold: 45,
        services: ['Deep Tissue Massage', 'Aromatherapy', 'Hot Stone', 'Body Wrap'],
        status: 'active',
        validity: '45',
        color: '#10B981',
    },
    {
        ...
        sessions: 5,
        sold: 23,
        services: ['Keratin Treatment', 'Hair Coloring', 'Deep Conditioning', 'Scalp Treatment', 'Blow Dry'],
        status: 'active',
        validity: '30',
        color: '#3B82F6',
    },
    {
        id: 5,
        name: 'Teen Beauty Starter',
        price: 350,
        oldPrice: 500,
        sessions: 3,
        sold: 18,
        services: ['Basic Facial', 'Manicure', 'Hair Trim'],
        status: 'draft',
        validity: '30',
        color: '#EC4899',
    },
    {
        ...
        sessions: 6,
        sold: 12,
        services: ['Couples Massage', 'Facial', 'Pedicure', 'Refreshments', 'Private Room'],
        status: 'active',
        validity: '90',
        color: '#EF4444',
    },
...
function ServicesPicker({ selected, onChange }: { selected: string[]; onChange: (svcs: string[]) => void }) {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
            <label
                style={{ fontSize: 'var(--text-sm)', fontWeight: 'var(--font-medium)', color: 'var(--text-primary)' }}
            >
                    ...
                    return (
                        ...
                        >
                            ...
                            <span
                                style={
                                    {
                                        color: isChecked ? 'var(--color-primary-700)' : 'var(--text-secondary)',
                                        fontWeight: isChecked ? 'var(--font-medium)' : 'var(--font-normal)',
...
export default function PackagesPage() {
    ...
    // API: fetch package sales
    const {
        data: apiPackages,
        loading: packagesLoading,
        error: packagesError,
        refetch: refetchPackages,
    } = useApiQuery<Record<string, unknown>[]>(() => salesApi.getPackageSales() as never, [], {
        fallbackData: initialPackages as never,
    });
    ...
    const filtered = useMemo(
        () => packagesData.filter(p => p.name.toLowerCase().includes(search.toLowerCase())),
        [packagesData, search]
    );
    ...
    const handleSell = async () => {
        ...
        try {
            await salesApi.createPackageSale({
                package_id: selectedPackage?.id,
                client: sellForm.client,
    ...
    const renderFormFields = () => (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
            <Input
                ...
                placeholder={t('sales.phPackageName')}
                value={form.name}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)' }}>
                <Input
                    label={t('sales.lblSessionsIncluded')}
                    type="number"
                    placeholder="0"
                    value={form.sessions}
                ...
                />
                <Select
                    label={t('sales.lblValidity')}
                    value={form.validity}
                    onChange={e => setForm({ ...form, validity: e.target.value })}
                    options={[
                        { label: `30 ${t('sales.lblDays')}`, value: '30' },
                        { label: `45 ${t('sales.lblDays')}`, value: '45' },
                        { label: `60 ${t('sales.lblDays')}`, value: '60' },
                        { label: `90 ${t('sales.lblDays')}`, value: '90' },
                    ]}
                />
            </div>
            <Select
                    ...
                    {colors.map(c => (
                        <button
                            ...
                            style={{
                                ...
                                cursor: 'pointer',
                                outline: form.color === c ? '2px solid var(--color-primary-200)' : 'none',
                                transition: 'all 0.15s',
                            }}
                        />
                    ))}
    ...
    return (
        <div style={{ ...s.page, direction: lang === 'ar' ? 'rtl' : 'ltr' }}>
            <div style={s.tabs}>
                <Link href="/sales" style={s.tab}>
                    <ShoppingBag size={16} /> {t('sales.lblServices')}
                </Link>
                <Link href="/sales/packages" style={{ ...s.tab, ...s.tabActive }}>
                    <Package size={16} /> {t('sales.lblPackages')}
                </Link>
            </div>
            ...
            >
                ...
                >
                    ...
                    <div>
                        <div style={{ fontSize: 'var(--text-xl)', fontWeight: 'var(--font-bold)' }} dir="ltr">
                ...
                >
                    ...
                    <div>
                        <div style={{ fontSize: 'var(--text-xl)', fontWeight: 'var(--font-bold)' }}>
                            {packagesData.filter(p => p.status === 'active').length}
                        </div>
                        <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)' }}>
                            {t('sales.kpiActivePackages')}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <DataGuard
                ...
                emptyTitle={t('sales.noPackagesFound')}
                emptyDescription={t('sales.noPackagesDesc')}
                        ...
                        <div key={pkg.id} style={s.card}>
                            <div style={s.cardHeader}>
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    <div style={s.prices}>
                                        <span style={s.price} dir="ltr">
                                            {pkg.price.toLocaleString()} {t('mkt.lblEGP')}
                                        </span>
                                        <span style={s.oldPrice} dir="ltr">
                                            {pkg.oldPrice.toLocaleString()} {t('mkt.lblEGP')}
                                        </span>
                                        <span
                                            style={{
                                                ...s.badge,
                                ...
                                <div style={{ marginInlineStart: 'auto' }}>
                                    <DropdownMenu
                                        ...
                                        items={[
                                            ...
                                            {
                                                label: t('sales.lblSellPackage'),
                                                icon: <ShoppingCart size={14} />,
                                                onClick: () => openSell(pkg),
                                            },
                                            {
                                                label: t('sales.lblBookPackage'),
                            ...
                            <div style={s.footer}>
                                <div style={s.stat}>
                                    <Users size={13} /> {pkg.sold} {t('mkt.lblSold')}
                                </div>
                                <div style={s.stat}>
                                    <Clock size={13} /> {pkg.validity} {t('sales.lblDays')}
                                </div>
                                <div style={s.stat}>
                                    <Star size={13} /> {pkg.sessions} {t('sales.lblSessions')}
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            <Modal
                ...
                footer={
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-3)' }}>
                        <Button variant="ghost" onClick={() => setIsSellOpen(false)}>
                            {t('rtn.btnBack')}
                        </Button>
                        <Button onClick={handleSell}>
                            <ShoppingCart size={16} /> {t('sales.btnConfirmSale')}
                        </Button>
                    </div>
                }
            >
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                        ...
                        >
                            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                                <div style={{ ...s.icon, background: selectedPackage.color, width: 40, height: 40 }}>
                                    <Package size={18} />
                                </div>
                                <div>
                                    <div
                                    ...
                                    <div
                                        ...
                                        dir="ltr"
                                    >
                                        {selectedPackage.price.toLocaleString()} {t('mkt.lblEGP')}
                                    </div>
                                </div>
                            </div>
                        </div>
                        <Select
                            ...
                            options={[
                                { value: '', label: t('sales.phSelectClient') },
                                ...mockClients.filter(c => c.value),
                            ]}
                        />
                        <Select
                        ...
                        />
                        <Input
                            label={t('sales.lblNotes')}
                            placeholder={t('sales.phSaleNotes')}
                ...
                footer={
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-3)' }}>
                        ...
                        <Button onClick={handleBook}>
                            <CalendarPlus size={16} /> {t('sales.btnConfirmBooking')}
                        </Button>
                    </div>
                }
            >
                {selectedPackage && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                                ...
                                <div>
                                    ...
                                    <div style={{ fontSize: 'var(--text-sm)', color: 'var(--text-tertiary)' }}>
                                        {selectedPackage.services.length} {t('sales.lblServices').toLowerCase()} ·{' '}
                                        {selectedPackage.sessions} {t('sales.lblSessions')}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)' }}>
                            <Input
                                label={t('sales.lblDate')}
                                type="date"
                                value={bookForm.date}
                                onChange={e => setBookForm({ ...bookForm, date: e.target.value })}
                            />
                            <Input
                                label={t('sales.lblTime')}
                                type="time"
                                value={bookForm.time}
