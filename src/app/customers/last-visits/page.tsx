...
const fallbackData = [
    {
        ...
        lastService: 'Hair Coloring',
        lastEmployee: 'Sara Ahmed',
        lastDate: '2026-03-16',
        daysSince: 0,
        followUp: false,
    },
    ...
    },
    {
        ...
        lastService: 'Swedish Massage',
        lastEmployee: 'Layla Hassan',
        lastDate: '2026-03-24',
        daysSince: 7,
        followUp: false,
    },
    ...
    },
    {
        ...
        lastService: 'Deep Tissue',
        lastEmployee: 'Reem Mohamed',
        lastDate: '2026-03-26',
        daysSince: 54,
        followUp: true,
    },
...
function getDaysBadge(days: number, t: (key: string) => string) {
    if (days <= 7)
        return {
            ...
            label: `${days} ${t('custVisits.daysAgo')}`,
        };
    if (days <= 30)
        return {
        ...
        if (apiVisits && apiVisits.length > 0) {
            return apiVisits.map((c, i) => {
                ...
                return {
                    ...
                    lastService: '-',
                    lastEmployee: '-',
                    lastDate: c.last_visit ?? '-',
                    daysSince,
                    followUp: daysSince > 14,
                };
            });
        }
        return fallbackData;
    ...
    return (
        <div style={{ ...s.page, direction: lang === 'ar' ? 'rtl' : 'ltr' } as React.CSSProperties}>
            <div style={s.tabs as React.CSSProperties}>
                ...
                <Link href="/customers/last-visits" style={{ ...s.tab, ...s.tabActive } as React.CSSProperties}>
            ...
            <DataGuard
                ...
                emptyTitle={t('custVisits.noVisits') || 'No visit records'}
                emptyDescription={
                    t('custVisits.noVisitsDesc') ||
                    'Customer visit history will appear here once appointments are completed.'
                }
                skeletonCount={5}
            >
                    ...
                    <thead>
                        <tr>
                            {[
                                ...
                                t('custProfile.lblPhone'),
                                t('custVisits.colLastService'),
                                t('custProfile.colEmp'),
                                t('custVisits.colLastVisit'),
                                t('custVisits.colDaysSince'),
                        ...
                        {filtered.map(row => {
                            ...
                            return (
                                <tr key={row.id}>
                                    ...
                                    <td
                                        style={s.td as React.CSSProperties}
                                        dir="ltr"
                                    ...
                                    <td style={s.td as React.CSSProperties}>
                                        {row.followUp && (
                                            ...
                                            >
                                                <AlertCircle size={12} className={lang === 'ar' ? 'ml-1' : 'mr-1'} />{' '}
                                                {t('custVisits.needsFollowUp')}
                                            </span>
                                        )}
                                    </td>
                                    <td style={s.td as React.CSSProperties}>
                                        <button style={s.callBtn as React.CSSProperties}>
                                            <Phone size={12} className={lang === 'ar' ? 'ml-1' : 'mr-1'} />{' '}
                                            {t('custVisits.callBtn')}
                                        </button>
                                    </td>
                                </tr>
                            );
