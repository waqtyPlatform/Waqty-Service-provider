...
- **Design Language v2 + full RTL & design-token migration (2026-06)** — a "Refined & premium" redesign across all ~110 routes. Type is **IBM Plex Sans / IBM Plex Sans Arabic / IBM Plex Mono**, self-hosted via `next/font` ([`src/app/layout.tsx`](src/app/layout.tsx)); a cool-slate neutral palette with a sparing `#00B166` green accent and a 4px spacing scale (`--space-*`) + radius/shadow tokens live in [`src/app/globals.css`](src/app/globals.css). This release completed a **full RTL logical-property migration**: every physical `left`/`right`, `margin-left`, `padding-left`, `text-align: left/right`, and `border-left/right` — in CSS Modules **and** inline `style={}` objects — was converted to logical properties (`inset-inline-*`, `margin`/`padding-inline-*`, `text-align: start/end`, `border-inline-*`), and `lang === 'ar' ? {…} : {…}` style conditionals were collapsed into single logical props, so the UI mirrors automatically under `dir="rtl"` (no horizontal overflow; search/leading icons flip sides correctly). A paired **design-token purity** pass replaced raw-px spacing/radius with `--space-*`/`--radius-*` and hardcoded hovers with color tokens. Verified live in both languages; `tsc` + `npm run build` clean on every route.
- **Real-API integration (2026-06)** — pages now read from the live Laravel backend at `NEXT_PUBLIC_API_BASE_URL` through `providerApi` + `useApiQuery`, with the existing mock arrays kept as graceful fallback for endpoints that aren't available yet.
- **Contract guard (CI)** — `src/lib/waqty_contract.ts` and `src/lib/platform_finance.ts` are **verbatim** copies of the canonical contract, fingerprinted against `src/lib/contract.lock` by `npm run contract:check` (runs in CI on every push to `main`). They are listed in `.prettierignore` so the pre-commit formatter never reformats them — **do not** hand-edit or reformat these two files; re-sync from the canonical instead.
