...
- **Domain model is app-owned (2026-06-10)** — the shared ecosystem contract was retired. `src/lib/waqty_contract.ts` and `src/lib/platform_finance.ts` are now owned by this app and may be edited freely; the real backend API is the future source of truth. The contract drift check (`contract:check`, `scripts/check-contract.mjs`, `src/lib/contract.lock`) was removed.
